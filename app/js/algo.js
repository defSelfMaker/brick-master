        ...
        if (!assumeInfinitePixelCounts) {
            ...
            if (availableStudMap[pixelHex] === 0) {
                ...
                oldHeapPixels.forEach((oldPixel) => {
                    ...
                    oldPixel.tentativeReplacementDistance =
                        tentativeReplacementDistance;
                });
            ...
            for (let kr = 0; kr < kernelHeight; kr++) {
                for (let kc = 0; kc < kernelWidth; kc++) {
                    if (kr != kernelRowMiddle || kc != kernelColMiddle) {
                        const pixelMatrixRow =
                            nextPixel.row - kernelRowMiddle + kr;
                        const pixelMatrixCol =
                            nextPixel.col - kernelColMiddle + kc;
                        const neighborhoodPixel = (pixelMatrix[
                            pixelMatrixRow
                        ] || {})[pixelMatrixCol];
                        if (
                    ...
                    for (let kc = 0; kc < kernelWidth; kc++) {
                        if (kr != kernelRowMiddle || kc != kernelColMiddle) {
                            ...
                            if (
                                ...
                                // add in error
                                const errorWeight =
                                    kernel[kr][kc] / errorDenominator;
                                neighborhoodPixel.pixelRGB = [0, 1, 2].map(
                                    (channel) =>
                                        clamp255(
                                            neighborhoodPixel.pixelRGB[
                                                channel
                                            ] +
                                                dequeuedPixelQuantizationError[
                                                    channel
                                                ] *
                                                    errorWeight,
                                        ),
                                ...
                                const tentativeReplacementDistance =
                                    colorDistanceFunction(
                                        neighborhoodPixel.pixelRGB,
                                        tentativeReplacementRGB,
                                    );
                                ...
                                if (
                                    oldReplacementRGB[0] !=
                                        neighborhoodPixel
                                            .tentativeReplacementRGB[0] ||
                                    oldReplacementRGB[1] !=
                                        neighborhoodPixel
                                            .tentativeReplacementRGB[1] ||
                                    oldReplacementRGB[2] !=
                                        neighborhoodPixel
                                            .tentativeReplacementRGB[2]
                                ) {
                    ...
                    forwardPixel.pixelRGB = [0, 1, 2].map((channel) =>
                        clamp255(
                            forwardPixel.pixelRGB[channel] +
                                (currentPixelQuantizationError[channel] *
                                    kernelEntry.val) /
                                    kernelDenominator,
                        ),
                    );
...
function resizeImageArrayWithAdaptivePooling(
    input2DArray,
    outputWidth,
    outputHeight,
    ...
    for (let h = 0; h < outputHeight; h++) {
        ...
        for (let w = 0; w < outputWidth; w++) {
            const startW = Math.floor(
                (w * input2DArray[1].length) / outputWidth,
            );
            const endW = Math.ceil(
                ((w + 1) * input2DArray[1].length) / outputWidth,
            );
            const startH = Math.floor((h * input2DArray.length) / outputHeight);
            const endH = Math.ceil(
                ((h + 1) * input2DArray.length) / outputHeight,
            );
...
function getRequiredPartMatrixFromSetPixelMatrix(
    ...
    for (let i = 0; i < partDimensions.length; i++) {
        ...
        for (let row = 0; row < setPixelMatrix.length - part[0] + 1; row++) {
            for (
                let col = 0;
                col < setPixelMatrix[0].length - part[1] + 1;
                col++
            ) {
...
const DEPTH_FILLER_PARTS = Object.keys(PLATE_DIMENSIONS_TO_PART_ID).map(
    (part) =>
        part
